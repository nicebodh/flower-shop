...
const formRef=ref()
const load=()=>{
  request.get('/shopkeeper/selectPage',{
    params:{
...
const add = () => {
  request.post("/shopkeeper/add", data.form).then(res => {
    if (res.code === '0') {
...
const update = () => {
  request.put("/shopkeeper/update", data.form).then(res => {
    if (res.code === '0') {
      data.formVisible = false
...
const del = (id) => {
  ...
  ElMessageBox.confirm('您确定删除吗？', '确认删除', {type: "warning"}).then(response => {
    request.delete('/shopkeeper/delete/' + id).then(res => {
